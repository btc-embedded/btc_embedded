"""
Example: TargetLink RBT + B2B Test Automation
"""
...
TL_MODEL_FILE = os.path.abspath('model/my_tl_model.slx')
TL_INIT_SCRIPT = os.path.abspath('model/init.m')
...
try:
    ...
    # 3. Create a fresh profile and import TargetLink architecture
    ep.post('profiles', message='Creating a new profile')
    ep.post('architectures/targetlink',
            {'tlModelFile': TL_MODEL_FILE, 'tlInitScript': TL_INIT_SCRIPT},
            message='Importing TargetLink architecture')
    ...
    # 7. Run Back-to-Back tests (TL MIL vs SIL)
    b2b_result = ep.post(f'scopes/{scope_uid}/b2b',
                         {'refMode': 'TL MIL', 'compMode': 'SIL'},
                         message='Running Back-to-Back test (TL MIL vs SIL)')
