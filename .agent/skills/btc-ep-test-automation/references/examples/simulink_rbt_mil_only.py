"""
Example: Simulink RBT MIL Only Test Automation
"""
...
EPP_FILE = os.path.abspath('my_sl_model.epp')
...
try:
    ...
    # 3. Load existing profile and update architecture
    ep.get(f'openprofile?path={EPP_FILE}', message='Loading source profile')
    
    # 4. Run architecture update
    ep.put('architectures', message='Running architecture update')
    
    # 5. Get scope UID and import test cases
    ...
    # 6. Run Requirements-Based Tests (RBT) - SL MIL only
    rbt_result = ep.post(f'scopes/{scope_uid}/test-execution-rbt',
                         {'execConfigNames': ['SL MIL']},
                         message='Running Requirements-Based Tests (SL MIL)')
    ...
    ep.put('profiles', {'path': RESULTS_EPP},
           message='Saving profile to results')
    
    # 8. Export JUnit XML results
    test_cases = ep.get('test-cases-rbt')
    dump_testresults_junitxml(
        rbt_results=rbt_result,
        test_cases=test_cases,
        scopes=scopes,
        project_name='my_sl_model',
